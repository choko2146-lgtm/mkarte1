...
- Step16で実装したパターンA UIブラッシュアップを土台に、色味、余白、情報の強弱をデザインシステムとして整理する。
- 画面ごとの個別調整で増えた色、drawable、余白、ボタン表現を共通ルールへ寄せる。
- 一般向け美容室アプリとして、清潔感、安心感、見やすさをさらに高める。

候補:

- `colors.xml` の役割名を整理し、メイン/サブ/補助/状態色の使い分けを明確にする。
- `dimens.xml` の余白、カード角丸、ボタン高さ、文字サイズを画面横断で再整理する。
- カード、入力欄、チップ、ボタン、下部ナビ、丸アイコン背景のdrawableを共通化する。
- ホーム、顧客一覧、写真一覧、写真詳細、カレンダーで情報の強弱と余白を再確認する。
- 色味がモックアップより強い/弱い箇所を実機で見ながら微調整する。

注意:

- Step16は完了扱い。
- Step17ではUIの設計整理を中心に扱い、Room DB構造、写真保存処理、MediaStore処理は変更しない。
- commit / push は未実施。
