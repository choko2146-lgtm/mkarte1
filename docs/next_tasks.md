...
- 共通dimenに寄せた余白、角丸、文字サイズが各画面で不自然に見えないこと。
- ホームで「写真を撮る」が最重要カードとして見え、最近撮影したお客様、新規顧客登録が控えめに整理されていること。
- ホームが今日の予定カードなしで、下部ナビまで1画面に収まること。
- 顧客一覧で顧客名、最終撮影日、写真枚数、フリガナ/電話番号の強弱が分かりやすいこと。
- 顧客一覧の検索、並び替えSpinner、カード間隔が崩れていないこと。
- 顧客詳細で顧客名、基本情報、撮影履歴・写真の各カードが読みやすいこと。
- 写真一覧で写真が主役になり、2列グリッド、顧客名、撮影日、メモが窮屈に見えないこと。
- 写真詳細で写真が大きく見え、左右矢印、メモ、編集、削除、プレビューが読みやすいこと。
- カレンダーに二重枠感がなく、撮影履歴カードと履歴行の余白が自然なこと。
- その他画面で住所録CSV出力が見つけやすく、未実装項目が補助的に見えること。
- 下部ナビが全画面で統一され、現在タブのみアクセント表示されること。
- 顧客詳細では顧客一覧タブ、写真詳細では写真一覧タブが選択状態で、タップ遷移も従来どおり動くこと。
- 写真保存、MediaStore表示、写真詳細左右移動、メモ保存、写真削除、顧客登録/編集/削除、顧客一覧ソートに回帰がないこと。

注意:

- Step17の実装は完了。次は実機でVisual Hierarchyと既存機能の回帰確認を行う。
- Room DB構造、写真保存処理、MediaStore処理は変更しない。
- commit / push は未実施。
